...
class Authentication {
  ...
  doLogin(): Promise<string> {
    return new Promise<string>((resolve, reject) => {
      ...
      if (!Objects.get(ServiceSettings, "AUTH.SERVER") || !Objects.get(ServiceSettings, "AUTH.ID") || !Objects.get(ServiceSettings, "AUTH.SECRET")) {
      ...
      /* Authenticate against account server */
      request.post(`${ServiceSettings.AUTH.SERVER}/v1/accounts/login`, {
        json: {
          email: Objects.get(ServiceSettings, "AUTH.ID"),
          password: Objects.get(ServiceSettings, "AUTH.SECRET")
        }
      },
